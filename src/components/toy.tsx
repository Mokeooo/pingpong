...
export default () => (
  <div className="container">
    ...
    <Scene style={{ borderRadius: 20,"blockSize":'600px' }} />
  </div>
)

function Scene(props: JSX.IntrinsicAttributes & CanvasProps) {
  const [accent, click] = useReducer((state) => ++state % accents.length, 0)
  const connectors = useMemo(() => shuffle(accent), [accent])
  return (
    <Canvas onClick={click} shadows dpr={[1, 1.5]} gl={{ antialias: false }} camera={{ position: [0, 0, 25], fov: 17.5, near: 1, far: 35 }} {...props}>
      ...
      <Environment resolution={512}>
        <group rotation={[-Math.PI / 3, 0, 1]}>
          <Lightformer form="circle" intensity={1} rotation-x={Math.PI / 2} position={[0, 5, -9]} scale={2} />
          ...
          <Lightformer form="circle" intensity={5} rotation-y={Math.PI / 2} position={[-5, -1, -1]} scale={2} />
          <Lightformer form="circle" intensity={6} rotation-y={-Math.PI / 2} position={[10, 1, 0]} scale={8} />
          <Lightformer form="circle" intensity={7} rotation-y={-Math.PI / 2} position={[10, 1, 0]} scale={8} />
...
function Model({ children, color = 'blue', roughness = 0, ...props }) {
  const ref = useRef()
  ...
  return (
    <mesh ref={ref} castShadow receiveShadow scale={10} geometry={nodes.connector.geometry}>
      <meshStandardMaterial metalness={Math.random().toFixed(1)} roughness={roughness} map={materials.base.map} />
      {children}
    </mesh>
  )
}
